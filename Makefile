...
PG_APPLICATION_URL ?= postgresql://$(ADMIN_POSTGRES_USER):$(ADMIN_POSTGRES_PASSWORD)@$(APPLICATION_POSTGRES_HOST):$(APPLICATION_POSTGRES_PORT)/$(APPLICATION_POSTGRES_DATABASE)
...
migrate: $(VENV)
	psql $(PG_ADMIN_URL) -c "CREATE ROLE $(APPLICATION_POSTGRES_USER) WITH ENCRYPTED PASSWORD '$(APPLICATION_POSTGRES_PASSWORD)' CREATEDB LOGIN;" || true
	psql $(PG_ADMIN_URL) -c "CREATE DATABASE $(APPLICATION_POSTGRES_DATABASE) WITH OWNER $(APPLICATION_POSTGRES_USER);" || true
